...
export class CustomersService {
    ...
    private _customers: BehaviorSubject<any> = new BehaviorSubject(null);
    ...
    addBusinessObject(data: any): Observable<any> {
        console.log(data);
        const token = localStorage.getItem('currentToken');
        return this._httpClient.post<any>(this.objectsBaseUri, data, {
            headers: {
    ...
    getBusinessObject(id: any, userType: any): Observable<any> {
        console.log("id="+id);
        console.log("userType="+userType);
        const token = localStorage.getItem('currentToken');
        return this._httpClient.get<any>(this.objectsBaseUri+"/"+id+"/"+userType, {
            headers: {
                // eslint-disable-next-line @typescript-eslint/naming-convention
                Authorization: 'Bearer ' + token
            }
    ...
    getCustomers(pageIndex?: any): Observable<any> {
        const token = localStorage.getItem('currentToken');
        if (pageIndex !== undefined) {
        } else {
            pageIndex = 1;
        }
        return this._httpClient.get<any>(this.customersBaseUri, {
            headers: {
                // eslint-disable-next-line @typescript-eslint/naming-convention
                Authorization: 'Bearer ' + token
            }
            ...
            tap((customers: any) => {
                this._customers.next(customers.content);
            })
        );
        /**
